// Module dependencies.

var events = require('events'),
    util = require('util'),
    joystick = require('joystick');


var LinuxConnector = function () {

    //private members
    var buffer = [];
    var _controllerConfig;

    var initBuffer = function () {
        var buttons = _controllerConfig.buttons;
        for(var i = 0; i < buttons.length; i++) {
            buffer[buttons[i].buttonBlock] = 0;
        }
    };

    //take a button event from joystick and update the buffer.
    var updateButtonBuffer = function (joystickData) {
        var buttons = _controllerConfig.buttons,
            button,
            bufferButton;
        //process buttons
        for (var i = 0; i < buttons.length; i++) {
            button = buttons[i];
            if (button.joystickNumber === joystickData.number) {
                bufferButton = buffer[button.buttonBlock];
                //the button is pressed.
                if (joystickData.value === 0) {
                    if (bufferButton & button.buttonValue) {
                        buffer[button.buttonBlock] = parseInt(bufferButton) - parseInt(button.buttonValue);    
                    }
                } else {
                    buffer[button.buttonBlock] = parseInt(bufferButton) + parseInt(button.buttonValue);

                }
            }
        }
    };

    var updateAxisBuffer = function (joystickData) {
        var analogSticks = _controllerConfig.analogSticks,
            analogStick;

                //process analogs
        for (var i = 0; i < analogSticks.length; i++) {
            analogStick = analogSticks[i];
            if (joystickData.number === analogStick.joystickYNumber) {
                buffer[analogStick.y] = Math.floor(joystickData.value / 125);
            } else if (joystickData.number === analogStick.joystickXNumber) {
                //need to invert values for x as it has a different pattern from node-hid
                buffer[analogStick.x] = Math.floor(joystickData.value / 125) * -1;
            }
             
        }
    };

    var emmitBufferData = function () {

        this.emit('data', buffer);

        //emit only at ~30fps;
        setTimeout(emmitBufferData.bind(this), 33);
    };
    this.getConnectedDevice = function (controllerConfig) {
        //defaulting to the first controller by passing 0;
        _controllerConfig = controllerConfig;

        //make sure the buffer is initiated;
        initBuffer();
        controller = new joystick(0, 3500, 350);

        controller.on('button', updateButtonBuffer.bind(this));
        controller.on('axis', updateAxisBuffer.bind(this));

        //wait for ~ 1second to start the emmit loop.
        setTimeout(emmitBufferData.bind(this), 1000);

        //refactor this, place in the constructor.
        return this;
    };
};

util.inherits(LinuxConnector, events.EventEmitter);
module.exports = LinuxConnector;
