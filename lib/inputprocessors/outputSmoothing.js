...
var outputSmoothing = function (smoothInput) {
    ...
    // Public methods
    that.readLastPosition = function (motionAxisName) {
        var axisBuffer = buffer[motionAxisName];
        return axisBuffer ? axisBuffer[axisBuffer.length -1] : null;
    };

    that.addToBuffer = function (motionAxisName, value) {
    ...
    that.smooth = function (motionAxisName, value) {
        that.addToBuffer(motionAxisName, value);
        var axisBuffer = buffer[motionAxisName],
            sum = 0,
            smoothedVal = value;
