...
var controller = function () {
	...
	var processFrame = function (error, data) {
		...
		//TODO: Refactor these for's to use arrays.
		//process the input data for button events.
		for (var button in controllerConfiguration.buttons) {
			this.processButtonEvents(button, data);
		}

		//process the input data for accelerometer and gyroscope
		for(var motionInput in controllerConfiguration.motionInputs) {
			this.processMotionEvents(motionInput, data);
		}
	...
	//raises pressed/relesed and analog events for a button.
	this.processButtonEvents = function (button, data) {
		//TODO: pass the button reference instead of the name to this function.
	...
	this.processMotionEvents = function (motionAxis, data) {
		var motion = {
			direction : data[controllerConfiguration.motionInputs[motionAxis].directionPin],
			value : data[controllerConfiguration.motionInputs[motionAxis].valuePin]
		};
		...
		if(isWithinVariance(buffer.motion[motionAxis], motion.value)) {
			this.emit(motionAxis + ':motion', motion);
		}

		buffer.motion[motionAxis] = motion.value;
	};
